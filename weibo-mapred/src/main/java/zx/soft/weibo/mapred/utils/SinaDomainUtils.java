...
import zx.soft.weibo.mapred.domain.Comments.Builder;
...
public class SinaDomainUtils {
	...
	public static Comments getWeiboReposts(SinaDomain sinaDomain) {
		Builder builder = new Comments.Builder(getLong(sinaDomain, "id"),
				dateChange(getString(sinaDomain, "created_at")), getString(sinaDomain, "text"), getUser(sinaDomain,
						"user").getId(), getUser(sinaDomain, "user").getScreen_name(), 1, getRetweetedWeibo(sinaDomain)
						.getId(), getRetweetedWeibo(sinaDomain).getUser().getId(), getRetweetedWeibo(sinaDomain)
	...
	private static User getUser(SinaDomain sinaDomain, String key) {
		...
		User user = new User.Builder(getLong(tmp, "id"), getString(tmp, "idstr"), getString(tmp, "screen_name"),
				getString(tmp, "name"), dateChange(getString(tmp, "created_at")))
		.setUclass(getInt(tmp, "class"))
		.setProvince(getInt(tmp, "province"))
		.setCity(getInt(tmp, "city"))
		.setLocation(getString(tmp, "location"))
		.setDescription(getString(tmp, "description"))
		.setUrl(getString(tmp, "url"))
		.setProfile_image_url(getString(tmp, "profile_image_url"))
		.setProfile_url(getString(tmp, "profile_url"))
		.setDomain(getString(tmp, "domain"))
		.setWeihao(getString(tmp, "weihao"))
		.setGender(getString(tmp, "gender"))
		.setFollowers_count(getInt(tmp, "followers_count"))
		.setFriends_count(getInt(tmp, "friends_count"))
		.setPagefriends_count(getInt(tmp, "pagefriends_count"))
		.setStatuses_count(getInt(tmp, "statuses_count"))
		...
		.setFollowing(getBoolean(tmp, "following"))
		.setAllow_all_act_msg(getBoolean(tmp, "allow_all_act_msg"))
		.setGeo_enabled(getBoolean(tmp, "geo_enabled"))
		.setVerified(getBoolean(tmp, "verified"))
		//.setVerified_type(Integer.parseInt(tmp.getFieldValue("verified_type").toString()))
		.setRemark(getString(tmp, "remark")).setPtype(getInt(tmp, "ptype"))
		.setAllow_all_comment(getBoolean(tmp, "allow_all_comment"))
		.setAvatar_large(getString(tmp, "avatar_large")).setAvatar_hd(getString(tmp, "avatar_hd"))
	...
	}//Tue Nov 03 22:52:06 +0800 2015       Fri Nov 06 01:25:09 CST 2015

	//	private static Date newDateChange(String date) {
	//		DateFormat dateFormat = new SimpleDateFormat("EEE MMM dd HH:mm:ss Z yyyy", Locale.ENGLISH);
	//		Date dat;
	//		try {
	//			dat = dateFormat.parse(date);
	...
	private static Weibo getRetweetedWeibo(SinaDomain sinaDomain) {
		SinaDomain tmp = null;
		Weibo weibo = null;
		if (sinaDomain.containsKey("retweeted_status") && (!sinaDomain.isEmpty())) {
			try {
				tmp = (SinaDomain) sinaDomain.getFieldValue("retweeted_status");
				weibo = new Weibo.Builder(getString(tmp, "id"), getString(tmp, "mid"), getString(tmp, "idstr"),
						dateChange(getString(tmp, "created_at"))).setText(getString(tmp, "text"))
						.setSource_allowclick(getInt(tmp, "source_allowclick"))
						.setSource_type(getInt(tmp, "source_type")).setSource(getString(tmp, "source"))
				...
				if (weibo.toString().equals("[]")) {
					logger.info("retweeted_status字段为空" + weibo.toString());
				}
			}
		}
		return weibo;
